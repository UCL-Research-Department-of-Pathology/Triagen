...
unless( -s $output_vcf ) {
    ...
    ( -s "$vep_path/vep" ) or die "ERROR: Cannot find VEP script variant_effect_predictor.pl in path: $vep_path\n";

    # Contruct VEP command using some default options and run it
    my $vep_cmd = "$vep_path/vep  --plugin CADD,$vep_data/CADD/whole_genome_SNVs.tsv.gz,$vep_data/CADD/InDels.tsv.gz --species $species --assembly $ncbi_build --offline --no_progress --no_stats --buffer_size $buffer_size --sift b --ccds --uniprot --hgvs --symbol --numbers --domains --gene_phenotype --canonical --protein --biotype --uniprot --tsl --pubmed --variant_class --shift_hgvs 1 --check_existing --total_length --allele_number --no_escape --xref_refseq --failed 1 --vcf --minimal --flag_pick_allele --pick_order canonical,tsl,biotype,rank,ccds,length --dir $vep_data --fasta $ref_fasta --format vcf --input_file $input_vcf --output_file $output_vcf";
    ...
    $vep_cmd .= " --polyphen b --af --af_1kg --af_1kg" if( $species eq "homo_sapiens" );
...
my @ann_cols = qw( Allele Gene Feature Feature_type Consequence cDNA_position CDS_position
    ...
    ExAC_AC_AN_FIN ExAC_AC_AN_NFE ExAC_AC_AN_OTH ExAC_AC_AN_SAS ExAC_FILTER CADD_PHRED CADD_RAW );
